...
from .naturalmatch_utils import consistency_loss, Get_Scalar
...
class NaturalMatch:
    def __init__(self, net_builder, num_classes, ema_m, lambda_u, lambda_e, \
        ...
        """
        class NaturalMatch contains setter of data_loader, optimizer, and model update methods.
        Args:
        ...
        super(NaturalMatch, self).__init__()
    ...
    def train(self, args, logger=None):
        ...
        time_p = p_model.mean()
        self.q_hat = (torch.ones([1, args.num_classes], dtype=torch.float) / args.num_classes).cuda()
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                current_logits = logits_x_ulb_w.detach()

                debiased_pseudo_label = F.softmax(current_logits - self.tau * torch.log(self.q_hat), dim=1)

                self.q_hat = update_q_hat(torch.softmax(current_logits, dim=-1), self.q_hat)

                # adaptive marginal loss
                delta_logits = torch.log(self.q_hat)

                logits_x_ulb_s = logits_x_ulb_s + self.tau * delta_logits

                # hyper-params for update
                time_p, p_model = self.cal_time_p_and_p_model(debiased_pseudo_label, time_p, p_model)
                unsup_loss, mask = consistency_loss(args.dataset, logits_x_ulb_s, debiased_pseudo_label,
                                                    time_p, p_model,
    ...
    def gaussian(self, x, mean, cov):
        return norm.pdf(x, mean, np.sqrt(cov))

    @torch.no_grad()
    def find_intersection(self, mean1, cov1, mean2, cov2):

        def equations(x):
            return self.gaussian(x, mean1, cov1) - self.gaussian(x, mean2, cov2)

        try:
            intersection_points = brentq(equations, mean1, mean2)

        except ValueError:
            intersection_points = (mean1 + mean2) / 2

        return intersection_points

    @torch.no_grad()
    def get_global_threshold(self, prob_w):
        device = prob_w.device
        probs = prob_w.cpu().numpy()
        if len(probs) < 2:
            return torch.mean(prob_w)

        gmm = GaussianMixture(n_components=2, covariance_type='full', random_state=0)
        gmm.fit(probs.reshape(-1, 1))

        # 打印拟合的参数
        means = gmm.means_.flatten()
        covariances = gmm.covariances_.flatten()

        intersection_ab = self.find_intersection(means[0], covariances[0], means[1], covariances[1])
        intersection_ab = torch.tensor(intersection_ab, device=device)
    ...
    def cal_time_p_and_p_model(self, logits_x_ulb_w, time_p, p_model):
        ...
        if time_p is None:
            time_p = self.get_global_threshold(max_probs)
        else:
            time_p = time_p * 0.999 + self.get_global_threshold(max_probs) * 0.001
        if p_model is None:
